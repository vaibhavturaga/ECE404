...
#encrypt
def ctr_aes_image(iv, imagefile, encryptfile, keyfile):
    genTables()
    ...
    outputfile = open(encryptfile, 'wb')
    ...
    message_bv = BitVector(filename = imagefile)  
    headers = message_bv.read_bits_from_file(112)
    headers.write_to_file(outputfile)   

    #BitVector(textstring ="AAA").write_to_file(outputfile)
    #outputfile.close()
    #exit()

    bv = iv
    while(message_bv.more_to_read):
        plaintext = message_bv.read_bits_from_file(128)
        #add padding
